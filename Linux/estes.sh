...
# Contributors: Tavin Turner, Abhinav Vemulapalli
# Referenced works: [1] J. Peña, Securing Debian Manual, 3rd ed. 2012, pp. 37-122. [2] A. Vemulapalli and T. Turner, Telluride. Highlands Ranch: Team Helix, 2018.

# [[[[[[[[[[[[[[[[ ORIGINAL ]]]]]]]]]]]]]]]]

# Script executed in su
if [[ $EUID -ne 0 ]]; then
  echo "This script must be run as root (su root)"
  exit 1
fi

apt-get install sudo libpam-tmpdir libpam-umask iptables libpam-cracklib curl ufw gufw

if [ $? = 100 ]; then
  echo "apt-get IS NOT WORKING. PLEASE FIX AND TEST BEFORE RERUNNING THIS SCRIPT"
  exit 1
fi

# Acquire system metrics and script variables
echo "PLEASE ANSWER THE FOLLOWING QUESTIONS BASED ON THE README; PRESS [ENTER] AFTER EACH QUESTION"
echo -n "Primary admin username: "
read sysadmin
echo -n "Secure Apache 2 (y/n)"
read apacheSecure
echo -n "Secure Nginx (y/n)"
read nginxSecure
echo -n "Secure MySQL (y/n)"
read mysqlSecure
echo -n "Secure PHP (y/n)"
read phpSecure
echo -n "Secure Bind9 (y/n)"
read bindSecure
echo -n "Secure VSFTP (y/n)"
read vsftpSecure

echo "Thank you very much. There should not be any more interrupts during the runtime of the script."

kernelversion=$(uname -r)

# Creating a directory for output
mkdir estesOutput
chmod 777 estesOutput
copydir=./estesOutput


# [[[[[[[[[[[[[[[[ FROM [1] ]]]]]]]]]]]]]]]]

# Set a LILO password [1:37-38]
# LILO
gawk -i inplace '/read-only/ { print; print "password=H3l!X?GoT3@m!"; print "restricted"; next }1' /etc/lilo.conf
/sbin/lilo
# GRUB
echo "Enabling password for grub boot loader"
...
echo "Set password for grub boot loader. Username: Admin Password: H3l!X?GoT3@m!"
# /[1:37-38]

# Disable root prompt on the initramfs [1:38]
# LILO
sed -i 's/#append = "".*/append = "panic=0"/g' /etc/lilo.conf
/sbin/lilo
# GRUB
sed -i 's/GRUB_CMDLINE_LINUX="".*/GRUB_CMDLINE_LINUX="panic=0"/g' /etc/default/grub
# /[1:38]

# Restricting console login access [1:39]
echo -e "auth\trequired\tpam_unix.so" >> /etc/pam.d/login

# Restricting the use of the Magic SysRq key [1:40]
echo "kernel.sysrq = 0" >> /etc/sysctl.conf
sysctl -p

# Providing secure user access in PAM [1:41]
echo -e "session\trequired\tpam_limits.so" >> /etc/pam.d/login
echo -e "session\toptional\tpam_tmpdir.so" >> /etc/pam.d/common-session

# Control of su in PAM [1:43]
groupadd wheel
usermod -a -G wheel $sysadmin
echo "auth  requisite pam_wheel.so  group=wheel debug" >> /etc/pam.d/su

# User login actions: edit /etc/login.defs [1:45]
...
# User login actions: edit /etc/pam.d/login [1:46]
sed -i '/auth/c\auth optional pam_faildelay.so delay=5000000'

# Restricting FTP: editing /etc/ftpusers [1:46]
awk -F : '{if ($3<1000) print $1}' /etc/passwd > /etc/ftpusers

# Disallow remote administrative access [1:47]
...
# Setting users umasks [1:49]
echo -e "session\toptional\tpam_umask.so\tumask=077" >> /etc/pam.d/common-session


# [[[[[[[[[[[[[[[[ FROM [2] ]]]]]]]]]]]]]]]]

# Update Firefox and remove legacy .mozilla files [2:118-121]
killall firefox
killall firefox-esr
mv ~/.mozilla ~/.mozilla.old
apt-get --purge --reinstall install firefox firefox-esr

# Firefox autoconfig
killall firefox
killall firefox-esr
cat Meta/syspref.js > /etc/firefox/syspref.js
cat Meta/syspref.js > /etc/firefox-esr/firefox-esr.js
echo "You should see: Alluha Helix I have completed My W0rk - syspref.js"
su -c 'firefox -new-tab about:config' $sysadmin | su -c 'firefox -new-tab about:config' $sysadmin
su -c 'firefox-esr -new-tab about:config' $sysadmin | su -c 'firefox-esr -new-tab about:config' $sysadmin

# Password policies [2:134-138]
sed -i '/pam_unix.so/ s/$/ remember=5 minlen=8 difok=3/g' /etc/pam.d/common-password
...
# Remove nullok [2:140-144]
sed -i 's/\<nullok_secure\>//g' /etc/pam.d/common-auth # Remove nullok
sed -i 's/\<nullok\>//g' /etc/pam.d/common-auth
sed -i 's/\<nullok\>//g' /etc/pam.d/common-password
sed -i 's/\<nullok_secure\>//g' /etc/pam.d/common-password

# Network Configuration [2:147-217]
...
# `sysctl` and disabling core dumps [2:240-274]
...
# Checking for prohibited files [2:337-356]
mediawarrant() {
  echo "Searching for $1 files out outputting to file for forensic question answers"
...
# Doing users. Hopefully this doesn't break. [2:303-358]
echo "Copying & parsing README"
touch $copydir/readme
chmod 777 $copydir/readme

readmeurl=`cat /home/$sysadmin/Desktop/README.desktop | grep -o '".*"' | tr -d '"'`
...
echo "Removed unauthorized users"


echo "Checking and removing unauthorized administrators"
...
for user in `cat $copydir/adminusers`; do
    ...
    echo The admin privileges of $user has been revoked
	fi
done
echo "Removed unauthorized admins"

echo "Changing passwords of all administrators"
...
echo "Changed admin passwords"
# /[2:303-358]

# Checking for any user who has a UID of 0 and is not a root and removing [2:361-403]
echo Checking for 0 UID users other than root and removing
...
# Services. Hoo hoo make sure you check [2:474-556]
echo "Evaluating compulsory status of pure-ftpd"
...
if [ $? = 0 ]; then
  ...
  echo "Installed and secured pure-ftpd"
else
  apt-get purge -y pure-ftpd
  apt-get autoremove
  echo "Removed pure-ftpd as not required"
fi

echo "Securing VSFTP or uninstalling."
if [ $vsftpSecure = "y" ] ; then
...
echo "Securing MySQL or uninstalling."
if [ $mysqlSecure = "y" ]; then
...
echo "Securing Apache or uninstalling."
if [ $apacheSecure = "y" ]; then
  a2enmod userdir
...
echo "Securing PHP or uninstalling."
if [ $phpSecure = "y" ]; then
...
echo "Securing Bind9 or uninstalling."
if [ $bindSecure = "y" ]; then
  # TODO: Implement the security for Bind9
  continue
else
  apt-get purge -y bind9
fi

echo "Securing nginx or uninstalling."
if [ $nginxSecure = "y" ]; then
  # TODO: Implement the security for nginx
  continue
else
  apt-get purge -y nginx
fi

echo "Evaluating compulsory status of SSHD"
...
if [ $? = 0 ]; then
    ...
    echo "Installed and secured SSH"
else
    apt-get purge -y openssh-server
    apt-get autoremove
    echo "Removed SSH as not required"
fi
# /[2:474-556]

# Removing services [2:648-650]
apt-get purge -y john* ophcrack minetest nmap wireshark netcat* polari rpcbind transmission-gtk empathy mutt freeciv kismet hydra* nikto* xinetd squid minetest p0f minetest-server
# /[2:648-650]
...
echo "In order to secure mysql run the following command: mysql_secure_installation"
echo "Make sure you go to the Application Checklists document and go through the checklist for PHP."
echo "The script has finished its runtime."
